 ...
 */
export function getBiddingOrder(hostId: string, seatOrder: string[], roundNumber: number): string[] {
  const n = seatOrder.length
  const hostIndex = seatOrder.indexOf(hostId)
  if (hostIndex === -1) throw new Error('Hostul nu se află în lista de locuri')
  const start = (hostIndex + roundNumber) % n
  const order: string[] = []
  for (let i = 0; i < n; i++) {
    order.push(seatOrder[(start + i) % n])
  }
  return order
}
 ...
 */
export function getValidBidOptions(cardsThisRound: number): Bid[] {
  return ['PASS', ...Array.from({ length: cardsThisRound }, (_, i) => i + 1)]
}

export function isBidValid(bid: Bid, cardsThisRound: number): boolean {
  if (bid === 'PASS') return true
  return Number.isInteger(bid) && bid >= 1 && bid <= cardsThisRound
}
