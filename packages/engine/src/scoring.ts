import { Bid, FINAL_ROUND, PeekChoice } from '@foaica/shared'

export function scoreForExactBid(bid: number): number {
  return 10 + (bid - 1) * 5
}
...
export function calculateRoundScore(
  ...
  if (roundNumber === FINAL_ROUND && peek) {
    ...
    if (peek === 'peek') return took ? FINAL_BID_PEEK_SUCCESS : FINAL_BID_PEEK_FAIL
    return took ? FINAL_BID_NO_PEEK_SUCCESS : FINAL_BID_NO_PEEK_FAIL
  }

  if (bid === 'PASS') {
    if (tricksWon === 0) return PASS_NO_TRICK_SCORE
    return tricksWon
  }

  if (tricksWon === bid) return scoreForExactBid(bid)
  if (tricksWon < bid) return FAILED_BID_SCORE
  return tricksWon
}

export function calculateBile(total: number): number {
  return total / 10
}
