 ...
 */
export function getBiddingOrder(hostId, seatOrder, roundNumber) {
    const n = seatOrder.length;
    const hostIndex = seatOrder.indexOf(hostId);
    if (hostIndex === -1)
        throw new Error('Hostul nu se află în lista de locuri');
 ...
 */
export function getValidBidOptions(cardsThisRound) {
    return ['PASS', ...Array.from({ length: cardsThisRound }, (_, i) => i + 1)];
}
export function isBidValid(bid, cardsThisRound) {
    if (bid === 'PASS')
        return true;
