...
const socketOwners = new Map<string, { roomCode: string; playerId: string }>()
...
function broadcastState(roomCode: string) {
  ...
  for (const socketId of sockets) {
    ...
    if (!owner || owner.roomCode !== roomCode) continue
...
function sendErrorToSocket(socket: Socket, message: string) {
  socket.emit('errorToast', message)
}

/** Programează avansul automat după afișarea câștigătorului unei mâini. */
function scheduleTrickAdvance(roomCode: string) {
  setTimeout(() => {
    const room = roomManager.getRoom(roomCode)
    if (!room || room.phase !== 'TRICK_RESULT') return
    try {
...
io.on('connection', (socket) => {
  ...
  socket.on('playCard', ({ roomCode, card }, cb) => {
    guarded(socket, roomCode, cb, (room, playerId) => {
      ...
      if (room.phase === 'TRICK_RESULT') scheduleTrickAdvance(roomCode)
    })
  })
  ...
  function guarded(
    ...
    action: (room: NonNullable<ReturnType<typeof roomManager.getRoom>>, playerId: string) => void
  ) {
    try {
